...
// Card de Gasto Fixo
const RecurringCard = ({ recurring, onEdit, onDelete, onRegisterPayment }) => {
  return (
    <motion.div
      layout
...
const RecurringTransactions = () => {
  ...
  const handleOpenDialog = (recurringItem = null) => {
    if (recurringItem) {
  ...
  const handleDelete = async (recurringItem) => {
  ...
  const handleOpenPaymentDialog = (recurringItem) => {
    setSelectedRecurring(recurringItem);
    setPaymentData({
      amount: recurringItem.amount.toString(),
      date: new Date().toISOString().split('T')[0]
      ...
      <Box sx={{ p: 3 }}>
        ...
        {recurring.length === 0 ? (
          <EmptyState
            ...
            description="Adicione suas despesas e receitas recorrentes para facilitar o controle"
          />
        ) : (
